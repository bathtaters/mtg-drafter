...
router.post('/upload', async function(req, res, next) {
  if (!req.files || !req.files.cubeFile) return res.reply({error: 'No files were sent'});
...
router.post('/rename', draftRules.rename(), validate, async function(req, res, next) {
  ...
  return res.reply({name: req.body.player.name});
...
router.post('/lands', draftRules.lands(), validate, async function(req, res, next) {
  if (!req.body.player || !req.body.landCount) return res.reply({});

  await req.body.player.setLandData(req.body.landCount);
  return res.reply({lands: req.body.player.getLandData()});
});
...
router.get('/isReady', function(req, res, next) {
  if (!req.body.session) return res.reply({});
  return res.reply({ refresh: req.body.session.status != draftStatus.pre });
});
