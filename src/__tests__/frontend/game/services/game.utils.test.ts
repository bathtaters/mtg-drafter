...
describe("getHolding", () => {
  ...
  const setPicks = (picks: number[]) =>
    picks.map((pick, idx) => ({ id: players[idx]?.id, pick }));

  it("all ones", () => {
    expect(getHolding(setPicks([1, 1, 1, 1]), 15, firstRound)).toEqual([
      1, 1, 1, 1,
    ]);
    expect(getHolding(setPicks([2, 2, 2, 2]), 15, firstRound)).toEqual([
      1, 1, 1, 1,
    ]);
    expect(getHolding(setPicks([5, 5, 5, 5]), 15, firstRound)).toEqual([
      1, 1, 1, 1,
    ]);
    expect(getHolding(setPicks([1, 1, 1, 1]), 15, secondRound)).toEqual([
      1, 1, 1, 1,
    ]);
    expect(getHolding(setPicks([5, 5, 5, 5]), 15, secondRound)).toEqual([
      1, 1, 1, 1,
    ]);
  });
  it("random values", () => {
  ...
  it("finshed picking", () => {
    expect(getHolding(setPicks([16, 16, 16, 16]), 15, firstRound)).toEqual([
      0, 0, 0, 0,
    ]);
    expect(getHolding(setPicks([15, 16, 16, 16]), 15, firstRound)).toEqual([
      1, 0, 0, 0,
    ]);
    expect(getHolding(setPicks([15, 16, 16, 16]), 15, firstRound)).toEqual([
      1, 0, 0, 0,
    ]);
    expect(getHolding(setPicks([16, 15, 15, 15]), 15, firstRound)).toEqual([
      0, 1, 1, 1,
    ]);
  });
  it("single player", () => {
