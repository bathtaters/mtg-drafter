...
export async function newCubeGame(
  { packSize, cardList, ...options }: CubeOptions,
  hostSessionId?: string
) {
  const packs = createPacks(
    cardList,
    options.playerCount * options.roundCount,
    packSize
  );
  return newGame({ ...options, packs }, hostSessionId);
}

export async function newBoosterGame(
  { packList, basics, ...options }: BoosterOptions,
  hostSessionId?: string
) {
  const packs = await buildBoosterPacks(packList, options.playerCount, basics);
  return newGame(
    { ...options, roundCount: packList.length, packs },
    hostSessionId
  );
}
  ...
  const game = await retry(() =>
    prisma.game.create({
      ...
      data: {
        ...
        packs: {
          create: options.packs.map((pack, index) => ({
            index,
            cards: { create: pack },
          })),
        },
