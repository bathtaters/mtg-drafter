...
export default async function buildCubeList(
  cardNames: string[],
  callback?: (count: number) => void,
  callPercent: number = 10
) {
  ...
  const cards = await prisma.card.findMany({
    where: { normalName: { in: cardNames } },
  });

  cardNames.forEach((cardName) => {
    const card = getBestMatch(
      cards.filter(({ normalName }) => normalName === cardName)
    );
