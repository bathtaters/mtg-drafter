...
export default async function buildBoosterPacks(
  boosterCodes: string[],
  playerCount: number,
  includeBasics = true
): Promise<PackCard[][]> {
  let packs: PackCard[][] = [],
    setCache: Record<string, SetBooster> = {};

  for (const boosterCode of boosterCodes) {
    const boosterData =
      boosterCode in setCache
        ? setCache[boosterCode]
        : await getFullSet(boosterCode);
    if (!boosterData)
      throw new Error(
        `Invalid booster code '${boosterCode},' or database is outdated.`
      );
...
function buildBoosterPack(booster: SetBooster, includeBasics = true) {
  const layout =
    booster.data.boosters.length === 1
      ? booster.data.boosters[0].contents
      : randomElemWeighted(
          booster.data.boosters,
          booster.data.boostersTotalWeight
        )?.contents;
  if (!Object.keys(booster.cards).length)
    throw Error(
      `Booster build for ${booster.setCode} failed due to missing card data.`
    );
  if (!layout) return [];
  ...
  logSheetNames &&
    console.log(
      ` > Sheet names [${booster.setCode}]: ${sheetsNames.join(", ")}`
    );
  ...
  sheetsNames.forEach((sheetName) => {
    ...
    if (!sheetData)
      throw new Error(`${sheetName} sheet not found in set ${booster.setCode}`);
    if ((layout[sheetName] ?? 0) < 1 || Object.keys(sheetData.cards).length < 1)
      throw new Error(
        `Invalid sheet card count [${booster.setCode}]: ${sheetName}`
      );

    let nextId: string,
      newCards: Card[] = [],
      count = layout[sheetName] ?? 0;
    for (let i = 0; i < count; i++) {
      do {
        // select unique card
        nextId = randomElemWeighted(sheetData.cards, sheetData.totalWeight);
      } while (
        !(nextId in booster.cards) ||
        (!sheetData.allowDuplicates &&
          newCards.some(({ uuid }) => nextId === uuid))
      );

      if (includeBasics || !landCards.includes(booster.cards[nextId].name))
        newCards.push(booster.cards[nextId]);
    }
    ...
    pack.push(
      ...newCards.map(({ uuid }) => ({ cardId: uuid, foil: sheetData.foil }))
    );
  });
