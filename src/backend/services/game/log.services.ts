      ...
      log: {
        where: filter && {
          AND: [
            ...
            filter.hideHost
              ? {
                  OR: [{ playerId: null }, { hostId: null }, { action: "ban" }],
                }
              : {},
            filter.hideWatchers
              ? {
            ...
            !filter.players
              ? {}
              : otherPlayers.some((player) => filter.players?.includes(player))
                ? {
                    OR: [
                      { playerId: { in: filter.players } },
                      { playerId: null /* otherPlayers = NULL */ },
                    ],
                  }
                : { playerId: { in: filter.players } },
          ],
        ...
        include: {
          card: {
            include: {
              card: { select: { scryfallId: true, name: true, img: true } },
            },
          },
        },
        skip,
...
export const getHasJoined = (gameId: Game["id"], sessionId: string) =>
  prisma.logEntry
    .count({
...
export const getHasViewed = (gameId: Game["id"], sessionId: string) =>
  prisma.logEntry
    .count({
      where: { gameId, sessionId, action: "view" },
      take: 1,
    })
    .then(Boolean);

export const getLogSize = (gameId: Game["id"]) =>
  prisma.logEntry.count({ where: { gameId } });

export const userInGame = (gameId: string, sessionId: string) =>
  prisma.logEntry
    .findFirst({
...
export const userIsWatcher = (gameId: string, sessionId: string) =>
  prisma.watcher
    .findFirst({
  ...
  try {
    const result = await prisma.$transaction([
      ...
      prisma.logEntry.create({
        data: { gameId: id, hostId, action: "leave", data: ALL_WATCHERS },
      }),
    ]);
    return Boolean(result[0].watchKey);
  } catch (error: any) {
    if (error.code === "P2025")
      console.error("Setting password: Game not found", id);
...
export async function setWatcher(
  ...
  hostId: Player["sessionId"],
  ignoreError: boolean
): Promise<Watcher | number>;
export async function setWatcher(
  gameId: Game["id"],
  sessionId: string,
  join: boolean,
  hostId: Player["sessionId"] = null,
  ignoreError = false
) {
  if (join) {
    ...
    return prisma
      .$transaction([
        ...
        prisma.logEntry.create({
          data: { gameId, sessionId, action: "join", data: name, hostId },
        }),
      ])
      .then((res) => res[0]);
  }
  ...
  if (ignoreError) {
    ...
    if (count)
      await prisma.logEntry.create({
        data: { gameId, sessionId, action: "leave", data, hostId },
      });
    return count;
  }
  return prisma
    .$transaction([
      prisma.watcher.delete({
        where: { sessionId_gameId: { gameId, sessionId } },
      }),
      prisma.logEntry.create({
        data: { gameId, sessionId, action: "leave", data, hostId },
      }),
