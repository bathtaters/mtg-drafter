...
export default function addPlayerListeners(
  ...
  socket.on("setName", async (playerId, name, byHost) => {
    try {
      ...
      // Update DB
      const player = await renamePlayer(
        playerId,
        name,
        currentSessionId,
      ...
      socket.emit(
        "errorMsg",
        `Error changing name: ${err.message || "Unknown"}`
      );
    }
  });
  ...
  socket.on("setStatus", async (playerId, status, byHost, callback) => {
    try {
      ...
      const sessionId =
        status === "bot" ? BOT : getExisitingSessionId(socket.request);
      if (sessionId == null) throw new Error("Missing user identity");

      const isBanned =
        sessionId !== BOT &&
        (await checkBanOrLock(game.id, sessionId, status === "join"));
      ...
      const hasViewed =
        !leave &&
        !gameIsEnded(game) &&
        (await getHasViewed(game.id, sessionId));
      if (hasViewed) throw new Error(viewedMsg);

      // Update DB
      const player = await setStatus(
        playerId,
        sessionId,
        leave,
        byHost ? currentSessionId : null
      ...
      socket.emit(
        "errorMsg",
        `Unable to ${status || "set status"}: ${err.message || "Unknown"}`
      );
      callback(undefined);
    }
  });
  ...
  socket.on("swapBoards", async (gameCardId, toBoard, callback) => {
    try {
      ...
      socket.emit(
        "errorMsg",
        `Error updating card position: ${err.message || "Unknown"}`
      );
    }
  });
  ...
  socket.on("setLands", async (playerId, lands, callback) => {
    try {
      ...
      socket.emit(
        "errorMsg",
        `Error setting basic lands: ${err.message || "Unknown"}`
      );
    }
  });
  ...
  socket.on("banSession", async (gameId, sessionId, unban, playerId) => {
    try {
      ...
      socket.emit(
        "errorMsg",
        `Error updating ${sessionId ? "user ban" : "game lock"} status: ${err.message || "Unknown"}`
      );
    }
  });
}
