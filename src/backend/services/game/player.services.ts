...
import {
  ...
  adaptDbPlayer,
  hasPack,
} from "backend/utils/game/game.utils";
...
const fullPlayer /* Prisma.PlayerInclude */ = {
  cards: {
    include: { card: { include: { otherFaces: { include: { card: true } } } } },
  },
};
...
export async function getPlayer(
  ...
  const playerIdx = playerList.findIndex(
  ...
  if (!player || !game.timerBase || game.round > game.roundCount)
  ...
  const timer = isPicking
    ? (startTime ?? Date.now()) +
      getTimerLength(packSize - player.pick + 1, game.timerBase) * 1000
    : null;

  if (timer !== null || player.timer)
    await retry(() =>
      prisma.player.update({ where: { id: player.id }, data: { timer } })
    );
  return { ...player, timer };
}
...
export async function setStatus(
  ...
  const player = await retry(() =>
    prisma.player.update({
      where: { id },
      data: { sessionId: leave ? null : sessionId },
    })
  );
...
export async function banPlayer(
  ...
  const ban: Partial<Ban & { unban: number }> = await retry(() =>
    !unban
      ? prisma.ban.create({ data: { gameId, sessionId, name } })
      : prisma.ban
          .deleteMany({ where: { gameId, sessionId } })
  ...
  if (unban && ban.unban !== 1)
    console.warn(
      `Unban resulted in unbanning ${ban.unban} rows (Expected: 1).`
    );
  return {
    gameId,
    playerId,
...
export const getBots = (gameId: Game["id"]) =>
  prisma.player.findMany({
    where: { gameId, sessionId: BOT },
    include: fullPlayer,
  });

export const getPlayerGame = (playerId: Player["id"]) =>
  prisma.player
    .findFirst({
      where: { id: playerId },
