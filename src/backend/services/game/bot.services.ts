...
export async function getBotPicks(gameId: Game['id']): Promise<PickData[]> {
    ...
    const packSize = await getRoundPackSize(game.id, game.round, game.roundCount, game.players.length)
    ...
    if (idxBots.some(({ idx }) => idx < 0)) {
        botPickLock = false
        throw new Error(`Bot(s) not found: ${idxBots.filter(({ idx }) => idx < 0).map(({ name }) => name).join(', ')}`)
    }

    // Iterate through all bots, preparing picks until no more picks can be made
    let hasPicked = true, picks: PickData[] = []
    while (hasPicked) {
        hasPicked = false
        for (const bot of idxBots) {
            ...
            while (holding-- > 0) {
                // Fetch current pack
                const pack = getCurrentPack({ player: bot, players, options, packs: packs as PackFull[] })
                if (!pack) {
                    botPickLock = false
                ...
                if (!card) {
                    botPickLock = false
                    throw new Error(`No cards left in pack ${pack.index} for bot: ${bot.name} (${bot.pick}).`)
                }
