...
import {
  fetchBulkUrl,
  adaptScryfallToImage,
  ImageData,
  isPreferredArt,
} from "../../utils/db/image.utils";
import { createMultiUpdate } from "../../utils/db/db.utils";

const multiUpdate = createMultiUpdate<ImageData>(
  "Card",
  ["scryfallId", "side"],
  ["img"],
  prisma
...
export default async function updateImages(
  ...
  const missingImgs = await prisma.card
    .findMany({
  ...
  enableLog &&
    console.log("Checking for", missingImgs.size, "missing Image URIs");
  enableLog && console.time("Image URIs");

  let count = 0;
  const batch = new Batcher(dbBatchSize, (data: ImageData[]) =>
    multiUpdate(data).then((c) => {
      count += c;
    })
  );

  await fetchJson<ScryfallCard>(
    imgUrl,
    async (data) => {
  ...
  while (preferred.length) {
    const res = await prisma.card.updateMany({
      ...
      where: {
        img: { not: null },
        scryfallId: { in: preferred.slice(0, upsertTxLimit) },
      },
    });
    count += res.count;
