...
export async function customDbModify<
  Table extends DbTables,
  T extends Awaited<ReturnType<(typeof prisma)[Table]["findFirstOrThrow"]>>,
>(
  table: Table,
  whereKeys: Extract<keyof T, string>[],
  updateKeys: Extract<keyof T, string>[],
  modifier: (entry: Partial<T>) => Partial<T> | null,
  selectKeys: Extract<keyof T, string>[] | null = null,
  enableLog = false,
  dbBatchSize = 5000
) {
  let count = 0,
    skip = 0;
  const tableName =
    modelMap[table.toLowerCase() as Lowercase<Prisma.ModelName>];

  const multiUpdate = createMultiUpdate<Partial<T>>(
    tableName,
    whereKeys,
    updateKeys,
    prisma
  );
  const batch = new Batcher(dbBatchSize, (data: Partial<T>[]) =>
    multiUpdate(data).then((c) => {
      count += c;
    })
  );
  ...
  enableLog &&
    console.log(
      "Running custom modify function on",
      total,
      `${table} entries.`
    );
  enableLog && console.time("Custom modify");

  const select = !selectKeys
    ? undefined
    : Object.fromEntries(
        [...whereKeys, ...updateKeys, ...selectKeys].map((key) => [key, true])
      );

  while (skip < total) {
    const entries = (await prisma[table as "card"].findMany({
      select,
      skip,
      take: dbBatchSize,
    })) as T[];
