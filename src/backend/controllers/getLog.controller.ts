...
export default async function apiHandler(
  req: NextApiRequest,
  res: NextApiResponse<LogFull | null>
) {
  ...
  if (!game?.id) {
    ...
    !canWatch(game, currentSessionId) &&
    currentSessionId !== game.hostId
  ) {
    console.error(
      "Error retrieving game log",
      url,
      "player",
      currentSessionId,
      "Player is not host or was not found in game!"
    );
    res.status(403).end();
  } else if (!game.log.length && !filter) {
    console.error(
      "Game log was empty at",
      url,
      "offset",
      offset,
      "/ size",
