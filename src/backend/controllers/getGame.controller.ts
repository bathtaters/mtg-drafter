...
import {
  ...
  getGame,
  getRoundPackSize,
} from "../services/game/game.services";
...
async function getGameProps(
  ...
  try {
    url = validation.url.parse(query.url);
  } catch (e) {
  ...
  if (isBanned)
    return { error: "", options: unregGameAdapter(game, sessionId), sessionId };

  try {
    packSize = await getRoundPackSize(
      game.id,
      game.round,
      game.roundCount,
      game.players.length
    ...
    return {
      ...
      options: unregGameAdapter(game, sessionId),
      sessionId,
    };
  }
  ...
  const player = (await getPlayer(
    ...
    packSize,
    now
  )) as PlayerFullTimer | null; // Convert type JSON value -> BasicLands

  const isHostOrWatcher =
    sessionIsHost(options, sessionId) || canWatch(options, sessionId);
  if (includePacks && !isHostOrWatcher)
    (packs as PackFull[]).forEach((pack) => {
      pack.cards = pack.cards.filter(({ playerId }) => !playerId);
      // Only allow players to see unpicked cards
    });

  const response: ServerProps =
    player || isHostOrWatcher
      ? {
          options,
          players,
          player,
  ...
  if (isHostOrWatcher)
    response.hasJoined = await getHasJoined(game.id, sessionId);
...
export async function apiHandler(
  req: NextApiRequest,
  res: NextApiResponse<ServerProps>
) {
  ...
  if (typeof props.error === "string") {
    if (props.error)
      console.error(
        ...
        req.query.url,
        ", player:",
        getReqSessionId(req, res),
        "--",
        props.error
      );
    res
      .status(props.error === NOTFOUND ? 404 : !props.error ? 403 : 400)
      .json(props);
  } else {
