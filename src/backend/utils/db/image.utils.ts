...
export const isPreferredArt = ({ layout }: ScryfallCard) =>
  layout !== "art_series";

export const adaptScryfallToImage = ({
  id,
  image_uris,
  card_faces,
}: ScryfallCard): ImageData[] =>
  !card_faces
    ? // Standard Card
...
export const fetchBulkUrl = (apiUrl: string) =>
  fetch(apiUrl, { headers: scryfallHeaders })
