...
const adaptSetToDb = ({
  ...
  releaseDate,
  block,
}: JsonSet): Prisma.CardSetCreateManyInput => ({
  code,
  name,
  releaseDate,
  block,
...
export function adaptSetDataToDb(setData: JsonSet) {
  ...
  const boosters: Prisma.BoosterCreateManyInput[] = !setData.booster
    ? []
    : Object.entries(setData.booster).map(([boosterType, data]) => ({
...
export const isBoosterSet = (setData: JsonSet): setData is JsonSet =>
  Boolean(setData.code && setData.name && "booster" in setData);

export const flattenObjects = <T extends Record<string, any>>(objArr: T[]) =>
  objArr.reduce(
    (result, next) => {
