...
export const createPlayers = (playerCount: number) =>
  [...Array(playerCount)].map((_, idx) => ({ name: `Player ${idx + 1}` }));
...
export function createPacks(
  ...
  if (packCount * packSize > cardList.length)
    throw new Error(
      `Not enough cards (${cardList.length}). Must have at least ${packCount * packSize} for this size game.`
    );
  let packs: PackCard[][] = [],
    next: PackCard[] = [];
