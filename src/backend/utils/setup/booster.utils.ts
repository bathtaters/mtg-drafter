...
export const COLOR_BASE = Object.fromEntries(
  [...Object.keys(Color), OTHER].map((c) => [c, 0])
);

export const compareKeys = <T extends {}>(
  obj: T,
...
export const getReplaceIndex = (
  cards: Card[],
  colors: Array<Color | typeof OTHER>
) => {
  const replaceId = randomElem(
    cards.filter((card: Card) => colors.includes(card.monoColor || OTHER))
  )?.uuid;
  return replaceId ? cards.findIndex(({ uuid }) => uuid === replaceId) : -1;
};

export const sortSheets = (layout: BoosterPack["contents"]) =>
  Object.keys(layout).sort(
    (a, b) => boosterSortOrder.indexOf(a) - boosterSortOrder.indexOf(b)
  );
