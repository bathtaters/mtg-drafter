...
export async function getName(
  ...
  if ((gameBanLogs as LogEntryFull)?.data)
    return (gameBanLogs as LogEntryFull).data;
  if (banLogs?.data) return banLogs.data;

  // Batch respond -- Priorities 8-9
  const logEntry =
    gameJoinLogs && "player" in gameJoinLogs && gameJoinLogs.player?.id
      ? gameJoinLogs
      : joinLogs;

  if (logEntry?.player?.id) {
    // If last action was a 'join', use the current name
    if (logEntry.action === "join" && logEntry.player.name)
      return logEntry.player.name;
...
export const getLastJoinSession = (
  sessionId: Player["sessionId"],
  gameId: Game["id"]
) =>
  prisma.logEntry
