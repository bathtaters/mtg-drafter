...
import {
  gameIsLocked,
  getNeighborIdx,
} from "components/game/shared/game.utils";
...
export const getMaxPackSize = (
  packCounts: { packIdx: number; _count: number }[],
  round: number,
  roundCount: number,
  playerCount: number
) => {
  ...
  for (let pack = rangeEnd - playerCount; pack < rangeEnd; pack++) {
    if (packCounts[pack]?.packIdx !== pack)
      throw new Error(`Pack ${pack} is missing!`);
...
export const adaptDbPlayer = <P extends Partial<DbPlayer>>(player?: P | null) =>
  (!player || typeof player.timer !== "bigint"
    ? player
    : {
        ...player,
...
export const getNextPlayerId = (
  playerId: Player["id"],
  game?: Pick<Game, "round" | "roundCount"> & {
    players: Array<{ id: Player["id"] }>;
  }
) => {
  ...
  return game.players[
    getNeighborIdx(game, game.players.length, playerIdx, true)
  ]?.id;
};

export const hasPack = (
  game: Pick<Game, "round" | "roundCount">,
  ...
  if (
    !game ||
    !players[playerIdx]?.pick ||
    game.round < 1 ||
    game.round > game.roundCount ||
    players[playerIdx].pick > packSize
  )
    return false;

  const neighborIdx = getNeighborIdx(game, players.length, playerIdx);
  return (
    neighborIdx === -1 || players[playerIdx].pick <= players[neighborIdx].pick
  );
};

export const unregGameAdapter = (
  { id, name, url, watchKey, banned }: Game,
  sessionId: string
) =>
  ({
    id,
    ...
    url,
    watchKey,
    locked: gameIsLocked(id, banned),
    isBanned: playerIsBanned({ id, banned }, sessionId),
  }) as PartialGame;
...
export function getTimerLength(cardCount: number, timerBase: number) {
  ...
  const {
    ...
    secOffset = 0,
    roundTo,
  } = timerOptions[timerBase]
    ? { ...defaultTimer, ...timerOptions[timerBase] }
    : defaultTimer;
