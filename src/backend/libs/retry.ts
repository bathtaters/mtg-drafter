...
// Strings
const msgRetry = (count: number, code: any) =>
    `Retrying (attempt: ${count}) after ${JSON.stringify(code)} error:`,
  msgMaxRetry = (count: number) =>
    `Exceeded maximum retries (${count}), throwing error.`,
  msgErrCode = (code: any) =>
    `Error code ${JSON.stringify(code)} not in retry codes, throwing error.`;
  ...
  while (true) {
    ...
    try {
      ...
      if (
        ...
        logRetry &&
          logRetry(
            count > maxRetries ? msgMaxRetry(maxRetries) : msgErrCode(err.code)
          );
        throw err;
      }
