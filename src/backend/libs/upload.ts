...
export default function getFileUploads(
  req: NextApiRequest,
  ...
  return new Promise<UploadObject>((resolve, reject) => {
    ...
    bb.on(
      "file",
      (fieldname: string, file: ReadStream & { truncated?: boolean }) => {
        file.on("data", (chunk) => data.push(Buffer.from(chunk)));
        file.on("error", (err) => reject(err));
        file.on("limit", () =>
          reject(
            `${fieldname} filesize limit reached (${limits?.fileSize} bytes)`
          )
        );
        file.on(
          "end",
          () => (files[fieldname] = Buffer.concat(data).toString("utf8"))
        );
      }
    );
    ...
    bb.on("fieldsLimit", () =>
      reject(`Fields limit reached (${limits?.fields})`)
    );
...
export const getSingleUpload = (req: NextApiRequest, sizeLimit?: number) =>
  getFileUploads(req, { files: 1, fileSize: sizeLimit }).then(
