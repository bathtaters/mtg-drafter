...
const commonOptionsObj = {
  name: z
    .string()
    .min(setupLimits.name.minLength)
    .max(setupLimits.name.maxLength)
    .trim(),
  playerCount: z
    .number()
    .int()
    .gte(setupLimits.players.min)
    .lte(setupLimits.players.max),
...
export const boosterOptions = z.object({
  ...
  packList: z
    .array(
      z
        .string()
        .min(setupLimits.boosterCode.min)
        .max(setupLimits.boosterCode.max)
        .includes(":")
    )
    .min(setupLimits.packs.min)
    .max(setupLimits.packs.max),
});
...
export const cubeOptions = z.object({
  ...
  roundCount: z
    .number()
    .int()
    .gte(setupLimits.packs.min)
    .lte(setupLimits.packs.max),
  packSize: z
    .number()
    .int()
    .gte(setupLimits.packSize.min)
    .lte(setupLimits.packSize.max),
  cardList: z.array(z.string().uuid()).max(setupLimits.cubeSize.max),
});

export const cubeFileOptions = z.object({
  [fileSettings.id]: file({
    count: 1,
    maxBytes: fileSettings.maxSize,
    typeList: [fileSettings.type],
  }),
});
