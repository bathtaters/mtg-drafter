...
export const EmptyPlayerContainer = ({
  className = "h-20",
}: {
  className?: string;
...
export const UserMarker = () => (
  <span
    className="inline-block ml-1 sm:mr-px tooltip tooltip-right"
    data-tip="You"
  >
...
export const HostMarker = () => (
  <span
    className="-mr-0.5 ml-0.5 mb-1 text-base/70 sm:text-lg tooltip tooltip-right"
    data-tip="Host"
  >
...
export const OppMarker = () => (
  <span
    className="mr-0.5 ml-1.5 mb-1 text-base sm:text-lg tooltip tooltip-right"
    data-tip="Opponent"
  >
...
export const UserHeader = ({
  ...
  <span>
    <span className="inline-block tooltip" data-tip="You">
      <UserIcon
        className={`${isConnected ? "fill-success" : "fill-error"} opacity-80 h-4 sm:h-5`}
      />
    </span>
    {isHost && <HostBadge />}
    {isBye && <ByeBadge />}
...
export const FullStatsWrapper = ({ children }: { children: ReactNode }) => (
  <div className="flex items-center gap-1 px-0.5">{children}</div>
);
...
export const StatsStyle = ({
  ...
  return isMini ? (
    <>
      <span
        data-tip={camelToTitle(type)}
        className="w-full h-full tooltip tooltip-left"
      >
        <Icon
          className={`${
      ...
      <span className="text-xs min-w-[1.1em] text-left mr-auto opacity-70">
        {count ?? "-"}
      </span>
    </>
  ) : (
    <>
      <Icon
        className={`inline-block ${type === "pick" ? "h-4 pr-0.5" : "h-5"} stroke-current fill-base-100`}
      />
...
export const PlayerNameEditor = (props: TextEditorProps) => (
  <TextEditor
    {...props}
    className="input-primary join-item text-lg sm:text-2xl "
  />
);
