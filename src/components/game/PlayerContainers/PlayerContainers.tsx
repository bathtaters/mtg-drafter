...
export const PlayerContainerSmall = ({
  player,
  holding,
  packSize,
  color,
    ...
    {hideStats ? (
      ...
      <>
        <StatsStyle
          ...
          type="pick"
          count={
            !player.pick || player.pick > packSize ? undefined : player.pick
          }
        />
        <StatsStyle
          isMini={true}
          type="holding"
          count={typeof holding === "number" ? Math.max(holding, 0) : holding}
        />
      </>
    )}
  </PlayerContainerStyle>
...
export const PlayerContainerFull = ({
  player,
  holding,
  packSize,
  isHost,
  ...
  return (
    <PlayerContainerStyle
      ...
      header={
        <UserHeader isHost={isHost} isBye={isBye} isConnected={isConnected} />
      }
      subtitle={
        <FullStatsWrapper>
          {hideStats ? (
            ...
            <>
              <StatsStyle
                type="pick"
                count={
                  !player.pick || player.pick > packSize
                    ? undefined
                    : player.pick
                }
              />
              ...
              <StatsStyle
                type="holding"
                count={
                  typeof holding === "number" ? Math.max(holding, 0) : holding
                }
              />
            </>
          )}
