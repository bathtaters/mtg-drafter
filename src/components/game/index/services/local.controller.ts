...
import {
  playerIsBanned,
  sessionIsHost,
} from "components/game/shared/player.utils";
...
export default function useLocalController(
  props: ServerProps,
  ...
  const [player, updatePlayer] = useState<
    Omit<PlayerFull, "timer"> | undefined
  ...
  const { timer, startTimer, resetTimer, storeTimer } = useTimerStore(
    props.player?.timer,
    props.now
  );
  ...
  const updateLocal = useCallback(
    (data: ServerProps) => {
      ...
      if (
        newPack &&
        data.player?.pick &&
        data.player.pick <= (data.packSize ?? 0)
      )
        storeTimer(data.player?.timer, data.now);
      else resetTimer();

      if (gameIsPaused(data.options) && data.now)
        data.options.pause += Date.now() - data.now;
  ...
  const nextRound: Local.NextRound = useCallback(
    (round) => {
  ...
  const pauseGame: Local.PauseGame = useCallback(
    (pause) => {
  ...
  const renamePlayer: Local.RenamePlayer = useCallback(
    (playerId, name) => {
  ...
  const pickCard: Local.PickCard = useCallback(
    (playerId, pick, passingToId) => {
      updatePlayers((list) =>
        updateArrayIdx(
          list,
          ({ id }) => id === playerId,
      ...
      if (passingToId && player?.id === passingToId)
        updatePack((pack) => {
  ...
  const setStatus: Local.SetStatus = useCallback(
    (playerId, sessionId, join, name) => {
      if (!playerId)
        return updateGame((game) =>
          !game || !("watchers" in game) || !game.watchers || !sessionId
            ? game
            : {
                ...game,
                watchers: join
                  ? game.watchers.concat({ sessionId, name: name || null })
                  : game.watchers.filter(
      ...
      updateSlots((s) =>
        join ? spliceInPlace(s, (pid) => pid === playerId) : s.concat(playerId)
      );
      updatePlayers(
        (list) =>
          list &&
  ...
  const banSession: Local.BanSession = useCallback(
    ({ unban, ...data }) => {
      if (!unban && data.sessionId)
        setStatus(data.playerId, data.sessionId, false);
