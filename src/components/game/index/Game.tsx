...
import {
  BodyWrapperStyle,
  SetPageTitle,
} from "components/base/styles/AppStyles";
    ...
    <>
      ...
      >
        <GameHeader
          ...
          openLands={toggleLandModal}
          openHost={toggleHostModal}
        ...
        <BodyWrapperStyle>
          <Loader
            data={game || 404}
            message={props.error || (isBanned && banMsg)}
          >
            {player ? (
              <GameBody
      ...
      {(!!loadingAll || !isConnected) && (
        <Overlay>
          <Spinner caption={!loadingAll ? "Reconnecting" : "Loading"} />
        </Overlay>
      )}
      ...
      {!!toggleLogModal && (
        <GameLogModal
          isOpen={logModal}
          setOpen={toggleLogModal}
          isHost={isHost}
          gameLog={gameLog}
          players={players}
          packs={packs}
          gameEnded={gameIsEnded(game)}
        />
      )}
      ...
      {!!toggleLandModal && (
        <LandsModal
          isOpen={landModal}
          setOpen={toggleLandModal}
          basics={player?.basics as BasicLands}
          onSubmit={setLands}
          cards={player?.cards}
          notify={newToast}
        />
      ...
      {!!toggleHostModal && (
        <HostModal
          ...
          renamePlayer={renamePlayer}
          setStatus={setStatus}
          setWatchPw={setWatchPw}
          dropWatcher={dropWatcher}
          banSession={banSession}
