...
export default function useBasicGameController(props: ServerProps, hostModal: boolean = false, setHostModal?: Dispatch<SetStateAction<boolean>>) {
  ...
  const socket = useSocket(
    gameURL(url),
    socketEndpoint(url),
    getGameListeners(local, newError, clearError, gameLog.fetchLatest, setHostModal),
    [local.game?.id, local.player?.id, local.isHost, local.isWatchPage, gameLog.fetchLatest],
