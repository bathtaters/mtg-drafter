...
import useAlerts, {
  AlertsReturn,
} from "components/base/common/Alerts/alerts.hook";
import { useFocusEffect } from "components/base/libs/hooks";
import useLocalController, {
  LocalController,
} from "./services/local.controller";
...
export default function useBasicGameController(
  props: ServerProps,
  hostModal: boolean = false,
  setHostModal?: Dispatch<SetStateAction<boolean>>
) {
  ...
  useFocusEffect(
    ...
    [local.game?.url, local.updateLocal, newError, socket.reconnect],
    refreshOnRefocusDelay
  );
...
export async function reloadData(
  ...
  try {
    ...
    if (res.status === 403 && res.data)
      true; // Don't throw error if user is banned
    else if (res.status !== 200 || !res.data || res.data?.error)
      throw new Error(`Cannot update data: HTTP error ${res.status}`);
