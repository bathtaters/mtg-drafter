...
export default function useGameController(props: ServerProps) {
  ...
  const { renamePlayer, setOptions, nextRound, pauseGame, pickCard, swapCard, setLands, setStatus, setWatchPw } = useGameEmitters(local, local.newError)
  ...
  return {
    ...local,
    renamePlayer, setOptions, nextRound, pauseGame, pickCard, swapCard, setLands, setStatus, setWatchPw,
