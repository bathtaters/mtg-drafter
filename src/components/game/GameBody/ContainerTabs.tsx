...
export const getTabBadges = (player?: PlayerFull, packCount?: number) =>
  player && {
    pack: packCount?.toString(),
    main: cardCounter(
      getBoard(player.cards, TabLabels.main).length,
      player.basics[TabLabels.main]
    ),
    side: cardCounter(
      getBoard(player.cards, TabLabels.side).length,
      player.basics[TabLabels.side]
    ),
  };

export default function ContainerTabs<Tabs extends string>({
  tabs,
  selectedTab,
  selectTab,
  hideTabs,
  ...
  const visibleTabs = !hideTabs
    ? Object.values(tabs)
    : Object.values(tabs).filter((tab) => !hideTabs.includes(tab));
