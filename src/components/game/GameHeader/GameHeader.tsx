...
} from "../PlayerContainers/PlayerContainers";
import {
  RoundCounter,
  LowerContainer,
  PlayerSeperator,
} from "./GameHeaderStyles";
...
export default function GameHeader({
  ...
  const {
    ...
    setEditingName,
    enableEdit,
  } = useGameHeader(game, players, playerIdx, isHost);
  ...
  return (
    <GameHeaderBase
      ...
      right={
        <RoundCounter
          label={roundCounter(gameStatus, game, !showDetail)}
          status={gameStatus}
        />
      }
    >
      {indexes && (
        <LowerContainer>
          ...
          {![undefined, indexes.prev, indexes.next].includes(indexes.opp) && (
            <>
              <PlayerSeperator
                passRight={players.length === 4 ? isRight : undefined}
                alt="..."
              />
