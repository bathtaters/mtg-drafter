...
} from "components/base/common/DropdownMenu";
...
const statusIcon: { [status in GameStatus]: ReactNode } = {
  start: (
    <span className="text-base sm:text-2xl fill-base-content ms ms-dfc-day" />
  ),
  end: (
    <span className="text-base sm:text-2xl fill-base-content ms ms-dfc-night" />
  ),
...
export const NewGameIcon = () => (
  <LogoIcon className="inline-block w-6 h-6 fill-current" />
);
...
export const GameHeaderWrapper = ({ children }: { children?: ReactNode }) => (
  <Header>
    <div className="w-full grid grid-cols-1 items-center gap-4">{children}</div>
  </Header>
);
...
export const GameTitle = ({
  label,
  ...props
}: CopyProps & { label?: ReactNode }) => (
  <>
    <div className="flex items-center justify-self-center">
      <h1 className="font-serif inline text-3xl sm:text-4xl text-right">
        {label}
      </h1>
      <CopyLink
        className="self-baseline tooltip-bottom"
        iconClass="w-5 ml-1"
        {...props}
      />
    </div>
...
export const RoundCounter = ({
  ...
  <div className="flex gap-2 justify-self-end items-center opacity-80">
    <span className="text-xs sm:text-base text-nowrap whitespace-nowrap">
      {label}
    </span>
...
export const MenuItemStyle = ({
  ...
  label: string;
  icon?: ReactNode;
  action?: MouseEventHandler<HTMLAnchorElement> | string | false;
}) =>
  typeof action === "undefined" ? undefined : typeof action === "string" ? (
