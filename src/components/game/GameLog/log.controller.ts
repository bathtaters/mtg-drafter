...
import useAdvancedFetch, {
  type FetchHandler,
} from "components/base/libs/advancedFetch";
...
export function useCardPopout(packs?: PackFull[]) {
  ...
  const setCard = (card?: GameCardPartial) =>
    showCard(
      card
        ? (packs?.[card.packIdx].cards.find(({ id }) => id === card.id) ?? card)
        : card
    );

  const [{ width }, setCardOptions] = useState<CardOptions>({
    width: "",
    showArt: true,
  });
  const { zoom, setZoom } = useToolbar({
    setCardOptions,
...
export default function useGameLog(
  url: Game["url"],
  playerData: BasicPlayer[]
) {
  const allPlayers = useMemo(
    () => playerData.map(({ id }) => id).concat(otherPlayers),
    [playerData]
  );
  ...
  const filter = useCallback(
    (entry?: LogEntryFull) => filterEntry(entry, players, actions, options),
    [players, actions, options]
  );

  const fetchLogs = useCallback<FetchHandler<LogEntryFull, LogParams>>(
    async (query, params) => {
      ...
      if (res.status !== 200 || res.error || res.data?.total == null)
        return { error: `Error <${res.status}> while fetching log.` };

      const data =
        params.offset == null && !params.isPreview
          ? res.data.log.toReversed()
          : res.data.log;
      return {
        data: data.map(adaptEntry),
  ...
  const download = async () => {
    const jsonData = await fetchAll().then((data) =>
      stringifyLogEntries(data, playerData)
    );
  ...
  useEffect(() => {
    fetch({ filter: { ...options, players, actions }, isPreview: true });
  }, [options, players, actions, fetch]);
