...
export default function GameLogModal({ isHost, isOpen, setOpen, gameEnded, ...props }: Props) {
  return (
    <LargeModal isOpen={isOpen} setOpen={setOpen} title="Game Log">
      <ModalToolbarWrapper>
        <LogToolbar gameLog={props.gameLog} players={props.players} gameEnded={gameEnded} isHost={isHost} />
      </ModalToolbarWrapper>
      <GameLog {...props} />
    </LargeModal>
  )
