...
export default function LogFilter<ID extends Key = string>({
  label,
  buttons,
  baseList,
  selected,
  ...
  return (
    <FilterWrapper label={label}>
      {buttons.map(({ id, name }, idx) => (
        <FilterButton
          key={id}
          isSelected={!selected || selected.includes(id)}
          setSelected={updateSelection(id)}
          color={label ? idx + offset : -1}
          inverse={invert}
        >
          {name ?? String(id)}
        </FilterButton>
      ...
      {!hideAll && (
        <FilterButton
          isSelected={true}
          setSelected={() => setSelected(!isAll ? baseList : [])}
          wide={!(buttons.length % 2)}
        >
          {!isAll ? "All" : "None"}
        </FilterButton>
      )}
