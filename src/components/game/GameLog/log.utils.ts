...
export const adaptEntry = <L extends Partial<LogEntry>>(entry: L) => ({
  ...entry,
  time: entry.time && new Date(entry.time),
});

export const filterEntry = (
  entry: LogEntryFull | undefined,
  players: FilterId[],
  ...
  if (
    options?.hideWatchers &&
    !entry.playerId &&
    watcherActions.includes(entry.action)
  )
    return false;
  if (options?.hideWatchers && entry.action === "view" && !entry.hostId)
    return false;
  return true;
};
...
export const objToString = (obj?: Record<string, any>) =>
  !obj
    ? ""
    : Object.entries(obj)
        .filter(([key]) => !endsInId.test(key))
        .map(
          ([key, val]) =>
            `${key}: ${key === "timerBase" ? timerText[val ?? 0]?.value || val : val}`
        )
        .join(", ");

export const getName = ({ action, data }: LogEntryFull) =>
  !watcherActions.includes(action)
    ? undefined
    : data === ALL_WATCHERS
      ? "All"
      : data;
...
export function stringifyLogEntries(
  ...
  const jsonData = entries
    .map((entry) => JSON.stringify(stringifyLogEntry(entry, players)))
    .join(",\n  ");
  return `[\n  ${jsonData}\n]\n`;
}

const stringifyLogCard = ({
  cardId: id,
  card: { scryfallId, name },
  foil,
  board,
}: NonNullable<LogEntryFull["card"]>) => ({
  name,
  foil,
  board,
  scryfallId,
...
const stringifyLogEntry = (
  { action, card, data, hostId, playerId, sessionId, time }: LogEntryFull,
  players: Record<string, Pick<BasicPlayer, "id" | "name">>
) => ({
  time,
  action,
