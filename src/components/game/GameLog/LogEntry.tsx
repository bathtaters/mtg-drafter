...
function FullLogEntry({
  ...
  const { time, action, data, hostId, sessionId, playerId, card, gameId } =
    entry;
  ...
  const isWatcher =
    action === "view"
      ? !hostId
      : !isGame && !playerId && (!!sessionId || data === ALL_WATCHERS);

  const gameData: Partial<Game> | undefined =
    action === "settings" ? data && JSON.parse(data) : undefined;

  return (
    <>
      {/* Date */}
      <EntryItem tip={logFullDate(time)} below={isFirst} right={true}>
        {logTimestamp(time)}
      </EntryItem>
      <EntrySpace />
          ...
          >
            {
              name
                ? name // Session name
                : playerIdx !== -2
                  ? players[playerIdx]?.name || playerId // Player name
                  : sessionId
                    ? "Watcher"
                    : "Game" // Generic entry
            }
      ...
      {/* Main Action */}
      <EntryItem
        tip={!isPrivate && card ? `${card.cardId} ${card.id}` : undefined}
        color={actionIdx}
        below={isFirst}
      >
      ...
      {/* Card */}
      {!isPrivate && card && (
        <EntryItem onClick={() => setCard(card)}>{card.card.name}</EntryItem>
      )}
      {action === "pick" && !card && (
        <EntryItem>
          <MissingCard />
      ...
      {action === "pause" && data && (
        <>
          <EntryItem>
            <i className="text-sm mr-2">(after {data}s)</i>
          </EntryItem>
          <EntrySpace />
        </>
      )}
      ...
      {/* By Host tag */}
      {hostId && (
        <EntryItem color={-1} inv={true} tip={hostId} below={isFirst}>
          by host
        </EntryItem>
      )}
      ...
      {/* By Watcher tag */}
      {action === "view" && !hostId && (
        <EntryItem color={-1} inv={true} tip={sessionId} below={isFirst}>
          by watcher
        </EntryItem>
      )}
      ...
      {action === "settings" && gameData?.id && (
        <EntryItem gameData={gameData} below={isFirst}>
          <GearIcon className="w-5 fill-current ml-2" />
        </EntryItem>
      )}
    </>
  );
