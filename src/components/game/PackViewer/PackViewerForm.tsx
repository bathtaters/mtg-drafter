...
export default function PackViewerForm({
  players,
  game,
  selectedPlayer,
  selectPlayer,
  ...
  return (
    <FormContainer>
      <SelectorContainer>
        ...
        >
          {players.map((player) => (
            <Fragment key={player.id}>{player.name}</Fragment>
          ))}
        </SelectorStyle>

        <SelectorStyle
          placeholder="Select Pack or Board"
          selected={viewType}
          setSelected={setViewType}
        >
          {upperLabels.map((val, idx) => (
            <Fragment key={allLabels[idx]}>{val}</Fragment>
          ))}
        </SelectorStyle>
      </SelectorContainer>
