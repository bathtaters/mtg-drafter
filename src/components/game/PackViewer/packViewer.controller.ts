...
import {
  ...
  logToPickInfo,
  sortByPickOrder,
} from "./packViewer.utils";
import { viewAuthError } from "assets/strings";
import { logFetchOptions } from "assets/constants";

const size = logFetchOptions.maxSize,
  params = { filter: { actions: ["pick"] } };

const formatErr = (message: string): ErrorAlert => ({
  message,
  title: "Cannot view cards",
  theme: "warning",
});

export function usePickInfo(
  game?: Partial<Game>,
  players?: BasicPlayer[],
  skip?: boolean
) {
  ...
  const updatePicks = useCallback(async () => {
    ...
    const res = await paginatedFetcher<LogEntryFull>(
      `/api/game/${game.url}/log`,
      { size, params, key: "log" }
    );
    setLoading(false);
...
export default function usePackViewer(
  ...
  const pack = useMemo(
    () =>
      !(packVisible && selectedPlayer && viewType && packs)
        ? undefined
        : viewType === TabLabels.pack
          ? sortByPickOrder(
              getPack(packs, selectedPlayer, round, game?.roundCount, players),
              pickInfo
            )
    ...
    (view: boolean) => {
      ...
      socket.emit(
        ...
        (success: boolean, reason?: string) => {
          ...
          success && onPackView?.();
          if (!success && newError)
            newError(formatErr(reason || viewAuthError.DEFAULT));
        }
      );
    },
    [
