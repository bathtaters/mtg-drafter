...
export const logToPickInfo = (
  logEntries: LogEntryFull[],
  players?: BasicPlayer[]
) =>
  logEntries.reduce((picks, entry) => {
    ...
    return {
      ...
      [entry.cardId]: {
        name: entry.playerId
          ? players?.find(({ id }) => id === entry.playerId)?.name
          : undefined,
...
export const getDeck = (
  ...
  packs.reduce(
    (deck, pack) => [
      ...
      ...pack.cards.filter(
        ({ playerId, board }) =>
          playerId === selectedPlayer && board === selectedBoard
      ),
    ],
    [] as PackFull["cards"]
  );

export const sortByPickOrder = (pack?: GameCardFull[], picks?: PickInfo) =>
  !pack || !picks
    ? pack
