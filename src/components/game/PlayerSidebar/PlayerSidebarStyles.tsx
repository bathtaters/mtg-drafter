...
} from "components/base/common/SidebarDrawer";
import UserIcon from "components/svgs/UserIcon";

export const SidebarDrawerStyle = ({
  children,
  ...props
}: Pick<
  SidebarProps,
  "isOpen" | "overlayClick" | "sidebarContent" | "children"
>) => (
  <SidebarDrawer
    {...props}
...
export const PlayerListWrapper = ({
  title = "Draft Order",
  children,
}: {
  title?: string;
  ...
  <div className="flex-grow flex flex-col gap-2 py-4 pr-4 overflow-y-auto scrollbar-secondary">
    <h2 className="text-center font-serif mb-4 font-normal opacity-80">
      {title}
    </h2>
    {children}
  </div>
...
export const ButtonSpacer = () => (
  <div className="invisible btn-sm md:btn-md btn-square ml-2" />
);
...
export const SidebarButton = ({
  ...
  <button
    type="button"
    onClick={onClick}
    className={`btn btn-secondary btn-outline btn-square btn-sm md:btn-md bg-base-200/80${
      active ? " swap-active" : ""
    }${
      hide ? " sm:invisible inline-grid" : ""
    } z-40 rounded-e-none border-r-0 mt-[4.5rem] md:mt-20 pointer-events-auto swap swap-rotate`}
  >
    <div className="swap-off">
      <UserIcon className="w-full p-2 md:p-3 fill-current" />
    </div>
...
export const Arrow = ({ isDown }: { isDown?: boolean }) => (
  <div className="w-8 px-1 overflow-hidden">
    <div
      className={`relative h-full ${isDown ? "-mt-16 " : "mt-16 "}transition-all duration-500`}
    >
      <svg
        viewBox="0 0 30 10000"
        className={`absolute w-full ${isDown ? "bottom-0" : "top-0"} fill-base-content/50`}
      >
        {typeof isDown === "boolean" && (
          <path
            d={
              isDown
                ? "M 30 0 L 30 10000 L 0 9960 L 15 9960 L 15 0 L 30 0 Z"
