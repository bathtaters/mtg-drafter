...
export default function PlayerSidebar({
  ...
  const { oppIdx, hideStats, passRight } = usePlayerSidebar(
    game,
    players,
    playerIdx
  );
  ...
  return (
    <SidebarDrawerStyle
      isOpen={isOpen}
      overlayClick={setOpen && (() => setOpen(false))}
      sidebarContent={
        <SidebarContainer
          isOpen={isOpen}
          button={
            playerIdx < 0 && !forceShow ? undefined : (
              <SidebarButton
                hide={!forceShow && players.length < 4}
                active={isOpen}
                onClick={setOpen && (() => setOpen((show) => !show))}
              />
            )
          }
        >
          ...
          <PlayerListWrapper title="Pick Order">
            {players.map((play, idx) => (
              <PlayerContainerSmall
                ...
                key={String(play.id)}
                isHost={
                  "hostId" in game ? game.hostId === play.sessionId : false
                }
