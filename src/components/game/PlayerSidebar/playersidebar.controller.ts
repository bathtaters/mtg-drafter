...
export default function usePlayerSidebar(
  game: GameProps["options"] | undefined,
  players: GameProps["players"],
  playerIdx: number
) {
  const gameExists = Boolean(game);
  const oppIdx = useMemo(
    () => (gameExists ? getOppIdx(playerIdx, players.length) : -1),
    [gameExists, playerIdx, players.length]
  );
