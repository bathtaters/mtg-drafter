...
export default function LandsModal(props: LandsModalProps) {
  ...
  return (
    <ModalWrapper
      isOpen={props.isOpen}
      title="Set Basic Lands"
      className="md:!max-w-3xl"
      buttons={
        <>
          <AutoLandsButton {...autoLandsProps}>Auto</AutoLandsButton>
          <ModalButton onClick={handleSave} className="btn-primary">
            Save
          </ModalButton>
    ...
    >
      <ColorsWrapper>
        <ColorLabels
          labels={["", "Main", "Side"]}
          className="text-2xl items-end font-bold font-serif"
        />
        ...
        {colorOrder.map((color) => (
          <ColorColumn
            label={color}
            key={color}
            main={localLands.main[color]}
            setMain={landChange("main", color)}
            side={localLands.side[color]}
            setSide={landChange("side", color)}
          />
        ))}
        <ColorLabels
          labels={landSums}
          className="italic font-sans items-start"
        />
      </ColorsWrapper>
    </ModalWrapper>
  );
...
const AutoLandsButton = ({
  ...
  <AutoLandsWrapper
    button={
      <ModalButton className="join-item btn-primary" onClick={onClick}>
        {children}
      </ModalButton>
    }
  >
    <AutoLandsInput
      label="Deck size"
      value={deckSize}
      onChange={(ev) => setDeckSize(+ev.currentTarget.value)}
      min="1"
      max="500"
    />
    <AutoLandsInput
      label="Sideboard"
      value={sideLands}
      onChange={(ev) => setSideLands(+ev.currentTarget.value)}
      min="0"
      max="50"
    />
  </AutoLandsWrapper>
