...
export default function useLandsModal({
  basics,
  cards,
  setOpen,
  onSubmit = () => {},
  ...
  const setAutoLands = () => {
    ...
    if (typeof lands === "string")
      return notify({ message: lands, theme: "warning" });
    setLocalLands(lands);
  };
  ...
  const landChange =
    (board: Board, color: keyof BoardLands) => (value: number) => {
      setLocalLands((lands) => ({
        ...lands,
        [board]: {
          ...lands[board],
  ...
  return {
    ...
    landSums: [
      "",
      getObjectSum(localLands.main),
      getObjectSum(localLands.side),
    ],
  };
}
