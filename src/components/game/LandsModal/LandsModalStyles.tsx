...
export function ColorsWrapper({ children }: { children: ReactNode }) {
  return (
    <div className="grid grid-cols-5 md:grid-cols-7 gap-4">{children}</div>
  );
}
...
export function ColorInputWrapper({
  ...
  return (
    <div className="join join-vertical">
      <label
        className={`join-item text-center ms-3x py-1.5 h-12 ${colorPip[label]} ${colorClass[label]}`}
...
export const ColorLabels = ({
  ...
  <div className={`join join-vertical ${className} hidden md:flex`}>
    {labels.map((label, idx) => (
      <div className="h-12 flex items-center" key={idx}>
        {label}
      </div>
    ))}
  </div>
...
export const AutoLandsInput = ({
  label,
  ...props
}: HTMLProps<HTMLInputElement>) => (
  <>
    <span className=" text-primary px-0 mr-2 justify-self-end">{label}</span>
    <NumberInput
      {...props}
...
export const AutoLandsWrapper = ({
  ...
  <div className="join mr-auto">
    ...
    <details className="dropdown join-item">
      <summary className="btn btn-primary rounded-l-none text-2xl py-1 px-2">
        ▸
      </summary>
      <div
