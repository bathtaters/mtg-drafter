      ...
      <Loader data={game}>
        ...
        >
          <PlayersWrapper>
            {players.map((player) => (
              <PlayerEntry
                key={player.id}
                player={player}
                isHost={player.sessionId === (game as Game)?.hostId}
          ...
          <Moderation
            ...
            banOne={banPlayer}
            kickOne={(playerId) =>
              setStatus(playerId, PlayerStatus.leave, true)
            }
          >
        ...
        <Collapser
          label="Watchers"
          isOpen={expanded === 3}
          toggle={toggleExpand(3)}
        >
          <PasswordForm
            label="Watch Password"
            disabled={!game?.watchKey}
            btnLabel="Set"
            emptyBtn={game?.watchKey ? "Disable" : "Disabled"}
          ...
          <WatchersContainer label="More Actions">
            ...
            <a className={linkClass} onClick={toggleExpand(2)}>
              View Ban List
            </a>
          </WatchersContainer>
        </Collapser>
