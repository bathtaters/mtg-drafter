...
import TextEditor, {
  type Props as TextEditProps,
} from "components/base/common/FormElements/TextEditor";
import IconToggle, {
  type Props as IconToggleProps,
} from "components/base/common/FormElements/IconToggle";
import RangeInput, {
  type Props as RangeInputProps,
...
export const NameWrapper = ({
  ...
  <div
    data-tip={tooltip}
    className={`flex-grow min-w-0 h-full border border-base-content/70${
      tooltip ? " tooltip" : ""
    }${
      formatLight ? " italic font-extralight text-sm text-opacity-80" : ""
    } ${className}`}
  >
    {children}
  </div>
...
export const Collapser = ({
  ...
  <div
    className="collapse collapse-plus bg-base-300 mt-2"
  ...
  >
    <input
      ...
      readOnly
    />
    <label className="collapse-title p-3 pl-4 min-h-0 text-lg justify-start gap-1 after:!top-3">
      {label || ""}
    </label>
...
export const NoPlayers = ({ children }: { children?: ReactNode }) => (
  <div className="w-full text-center italic font-extralight text-sm opacity-70">
    {children}
  </div>
);
...
export const TitleEditor = (props: TextEditProps) => (
  <div className="font-serif text-xl bg-base-300 border border-secondary rounded-lg flex-grow">
    <TextEditor
      {...props}
      className="input-secondary join-item bg-base-300 text-xl"
    />
  </div>
);

export const PauseButton = (
  props: Omit<IconToggleProps, "className" | "children">
) => (
...
export const HostTimerInput = (
  props: Pick<RangeInputProps, "value" | "setValue" | "min" | "max">
) => (
  <RangeInput
    keys={timerText}
    caption={
...
export const NameEditor = (props: TextEditProps) => (
  <NameWrapper className="bg-base-300 rounded-r-lg">
    <TextEditor
      {...props}
      className="input-secondary join-item text-sm sm:text-base"
    />
  </NameWrapper>
);
...
export const WatchersContainer = ({
  label,
  children,
}: {
  label?: ReactNode;
  ...
  <div className="pt-4 w-full">
    ...
    <div className="w-full flex justify-around items-center gap-2font-light">
      {children}
    </div>
  </div>
);
...
export const BanNameWrapper = ({
  ...
  <NameWrapper
    tooltip={tooltip}
    formatLight={!children}
    className={`bg-base-100 py-1 text-left ${isBanned ? "text-error " : ""}${
...
export const LockButton = ({
  locked,
  ...props
}: { locked?: boolean } & HTMLProps<HTMLButtonElement>) => (
  <div className="w-full flex justify-center p-2">
    <button
      {...props}
      type="button"
      data-tip={
        locked ? "Allow players to join" : "Prevent new players from joining"
      }
      className={`w-32 tooltip btn btn-sm ${
        locked
          ? "btn-success tooltip-success"
          : "btn-outline btn-error tooltip-error"
