      ...
      <ToolbarCollapse button={ToolbarButton} defaultOpen={false}>
        <ToolbarContainer>
          <CardSort
            label="Sort cards"
            selected={sort}
            setSelected={(idx) => setSort(+(idx ?? 0))}
          >
            {sortList.map((sortType, idx) => (
              <Fragment key={idx}>{sortType}</Fragment>
            ))}
          </CardSort>
          ...
          <CardArtWrapper>
            <ArtSize
              ...
              setValue={setZoom}
              min={0}
              max={cardZoomLevels.length - 1}
            />

            <ArtToggle
              label={art ? "Hide art" : "Show art"}
              value={art}
              setValue={setArt}
            />
          </CardArtWrapper>
