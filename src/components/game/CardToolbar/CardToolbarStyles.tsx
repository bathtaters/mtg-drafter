...
} from "components/base/common/Collapse";
import Selector, {
...
export const ToolbarCollapse = (props: CollapseProps) => (
  <CollapseContainer
    title="View Options"
    className="-mt-7 pt-7"
    buttonClass="absolute right-0 top-4 p-2 btn-ghost swap-rotate"
    {...props}
  />
);
...
export const ReloadButton = (
  ...
  <button
    ...
    data-tip="Reload"
    type="button"
    className="absolute left-0 top-4 btn btn-circle btn-sm tooltip tooltip-secondary
    border-secondary hover:border-secondary text-secondary bg-secondary-content"
  >
    <ReloadIcon className="w-5 fill-current m-auto" />
  </button>
);
...
export const ToolbarButton = [
  <CardIcon
    key="open"
    innerIcon="X"
    className="h-8 fill-secondary text-secondary-content"
  />,
  <CardIcon
    key="close"
    innerIcon="gear"
    className="h-8 fill-secondary-content text-secondary"
  />,
];
...
export const CardSort = (props: SelectorProps<number>) => (
  <div className="flex">
    ...
    <Selector
      className="select-secondary select-sm tooltip-secondary"
      {...props}
    />
  </div>
);
...
export const ArtToggle = ({
  label,
  value,
  setValue,
}: Pick<ToggleProps, "label" | "value" | "setValue">) => (
  <IconToggle
    label={label}
    value={value}
    setValue={setValue}
