...
export type ToolbarProps = {
  setCardOptions: Dispatch<SetStateAction<CardOptions>>;
  notify: (alert: ToastAlert) => any;
  clickReload?: () => void;
};
...
export default function useToolbar({ setCardOptions, notify }: ToolbarProps) {
  ...
  useEffect(() => {
    const updateMax = () => {
      (maxZoom.current = getMaxZoom()) < zoom && setTempZoom(maxZoom.current);
    };
  ...
  useEffect(() => {
    setCardOptions((opt) => ({ ...opt, showArt: art }));
  }, [art, setCardOptions]);
  useEffect(() => {
    setCardOptions((opt) => ({ ...opt, sort: sortKeys[sort] }));
  }, [sort, setCardOptions]);
  useEffect(() => {
    setCardOptions((opt) => ({ ...opt, width: cardZoomLevels[zoom] }));
  }, [zoom, setCardOptions]);
