...
export const canWatch = (
  game?: { watchers?: { sessionId?: string | null }[] },
  session?: string
): boolean =>
  !game?.watchers || !session
    ? false
    : game.watchers.some(({ sessionId }) => session === sessionId);

export const gameIsPaused = (
  game?: Partial<Game>
): game is Game & { pause: number } => typeof game?.pause === "number";

export const gameIsEnded = (game?: Partial<Game>): boolean =>
  typeof game?.round === "number" && game.round > (game.roundCount || 0);

export const gameIsLocked = (id?: Game["id"], banned?: Game["banned"]) =>
  !id || !banned
    ? false
    : banned.some(({ gameId, sessionId }) => !sessionId && gameId === id);

export const getBoard = <C extends GameCard>(playerCards: C[], board: Board) =>
  playerCards.filter(({ board: cardBoard }) => board === cardBoard);
...
export const passingRight = ({ round, roundCount }: Partial<Game>) =>
  typeof round !== "number" || round < 1 || round > (roundCount || 0)
    ? undefined
    : round % 2 === 1;

export const getPlayerIdx = (
  players: Pick<BasicPlayer, "id">[],
  player?: Pick<BasicPlayer, "id"> | null
) => (!player?.id ? -1 : players.findIndex(({ id }) => id === player.id));

export const getNeighborIdx = (
...
export const getPackIdx = (
  game: Pick<Game, "round" | "roundCount"> | undefined,
  players: Pick<BasicPlayer, "id" | "pick">[],
  player: Pick<BasicPlayer, "id"> | null,
  forcePick?: number
) => {
  ...
  if (forcePick == null) {
    ...
    if (
      neighborIdx !== -1 &&
      players[playerIdx].pick > players[neighborIdx].pick
    )
      return -1;
  ...
  return (
    (game.round - 1) * players.length +
    mod(playerIdx + forcePick * (passingRight(game) ? -1 : 1), players.length)
  );
};

export const getRoundPackSize = (
  packs: PackMin[],
  playerCount: number,
  game?: Partial<Game>
) => {
  ...
  let packSize = 0,
    offset = (game.round - 1) * playerCount;
  for (let i = 0; i < playerCount; i++) {
    if (packs[offset + i]?.cards.length || 0 > packSize)
      packSize = packs[offset + i].cards.length;
  }
  return packSize;
};

export const getHolding = (
  players: Pick<BasicPlayer, "pick">[],
  packSize: number,
  game?: Partial<Game>
) =>
  !game || !players.length || typeof game.round !== "number"
    ...
    : players.map(({ pick }, i) => {
        ...
        const neighborPick =
          players[getNeighborIdx(game, players.length, i)]?.pick;
        if (typeof neighborPick !== "number") return 0;

        return Math.min(neighborPick, packSize) - pick + 1;
      });

export const getSlots = (players?: BasicPlayer[]) =>
  players
    ? players.filter(({ sessionId }) => !sessionId).map(({ id }) => id)
    : [];

export const getCanAdvance = (
  game?: Partial<Game>,
  players: BasicPlayer[] = [],
  holding: number[] = []
) =>
  ...
  typeof game.round === "number" &&
  (game.round < 1
    ? players.every(({ sessionId }) => sessionId)
    : holding.every((h) => !h));

export const getCurrentPack = ({
  packs,
  options,
  player,
  players,
}: Pick<ServerProps, "options" | "packs" | "player" | "players">) => {
  const pack =
    packs?.[getPackIdx(options as Game, players ?? [], player ?? null)];
  return (
    pack && {
      ...pack,
      cards: pack.cards.filter(({ playerId }) => !playerId),
    }
  );
};
