...
import { CardCounter, ContainerHeaderStyle, ContainerLabelStyle, LandContainerStyle, LandCounterStyle } from "./CardContainerStyles"
...
export default function ContainerHeader({ label, count, lands }: { label: ReactNode, count?: number, lands?: BoardLands }) {
  return (
    <ContainerHeaderStyle>
      ...
      <LandContainerStyle>{ lands && colorOrder.map((c) => <LandCounter key={c} color={c} count={lands[c]} />)}</LandContainerStyle>
    </ContainerHeaderStyle>
  )
}