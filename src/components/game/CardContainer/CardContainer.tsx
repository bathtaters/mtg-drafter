...
type Props = {
  ...
  name?: string | null;
  cards?:
    | Pick<GameCardFull, "id" | "foil" | "card" | "playerId">[]
    | "roundEnd";
  lands?: BoardLands;
  loading?: number;
  paused?: boolean;
...
export default function CardContainer({
  ...
  return (
    <CardContainerWrapper
      ...
      isPrimary={type !== TabLabels.pack && type !== "select"}
      onClick={onBgdClick}
    >
      {paused && <PausedGame />}
      <CardsWrapper hideCards={loading > 0}>
        {overrideBody ? (
          overrideBody
        ) : loading < 0 ? null : typeof cards === "string" ? (
          <RoundOver />
        ) : !cards ? (
          <NoPack />
        ) : !count ? (
          ...
          cards
            .toSorted((a, b) =>
              packSort[cardOptions.sort ?? sortKeys[0]](a.card, b.card)
            )
            .map(({ id, foil, card }) => (
              <Card
                card={card}
      ...
      {name && (type === TabLabels.main || type === TabLabels.side) && (
        <NotLive />
      )}
