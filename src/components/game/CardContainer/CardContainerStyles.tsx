...
export const NotLive = () => (
  <div className="text-sm font-extralight italic text-base-content opacity-60 mt-8">
...
export const LoadingPack = ({
  ...
  loading == null ? (
    ...
    <EmptyStyle>
      ...
      <div className="mt-8">
        {loading < 1 ? "Looking for pack" : "Loading cards"}.
      </div>
    </EmptyStyle>
  );
...
export const CardContainerWrapper = ({
  ...
  <div
    onClick={onClick}
    className={`relative rounded-lg shadow-md shadow-black mb-8 flex-grow ${
      isPrimary
        ? "text-primary bg-primary-content/40"
        : "text-secondary bg-secondary-content/40"
    }`}
  >
...
export const LandContainerStyle = ({
  onClick,
  children,
}: {
  onClick?: MouseEventHandler;
  ...
  <div className="ml-auto col-span-2">
    <button
      type="button"
      onClick={onClick}
      className="relative z-20 flex justify-end gap-0 md:gap-2"
    >
      {children}
    </button>
  </div>
...
export const CardCounter = ({ text = "–" }: { text?: string }) => (
  <span className="badge badge-outline md:badge-lg align-top ml-2 whitespace-nowrap text-sm">
    {text}
  </span>
);

export const LandCounterStyle = ({
  className = "",
  children,
}: {
  className?: string;
  ...
  <span className={`badge badge-sm py-2 md:badge-lg ${className} flex gap-1`}>
    {children}
  </span>
);

export const LandButton = () => (
  <span className="btn btn-circle btn-sm btn-outline">
    <i className="ms ms-land text-base" />
  </span>
);
...
export const CardsWrapper = ({
  ...
  <div
    className={`${hideCards ? "hidden" : "flex"} gap-4 flex-wrap justify-center`}
  >
    {children}
  </div>
);
