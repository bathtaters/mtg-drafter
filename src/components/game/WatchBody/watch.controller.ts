...
export default function useWatchController(
  ...
  const tabProps = useTabController<WatcherTabs>(
    isHost && !hasViewed ? WatcherTabs.join : WatcherTabs.log,
    game,
    players,
    false
  );
  const packViewData = usePackViewer(
    packs,
    tabProps.pickInfo?.data,
    players,
    game,
    sessionId,
    socket.socket,
  ...
  const logout = isHost
    ? undefined
    : () => {
        if (!game?.id || !sessionId || !socket.isConnected)
          return newError({
            title: "Logout Failed",
            theme: "warning",
            message: !game?.id
  ...
  useEffect(() => {
    ...
    const updateBan: GameServerToClient["updateBan"] = ({
      sessionId: session,
      unban,
