...
function copyArrObj<T extends {}, KeyList extends keyof T>(
  obj: T,
  onlyKeys: Readonly<Array<keyof T>>
) {
  ...
  for (const key of onlyKeys) {
    ...
    copy[key] = (
      Array.isArray(obj[key]) ? [...(obj[key] as any[])] : obj[key]
    ) as T[keyof T];
  }
...
const flattenArrays = <T extends Record<string, string[]>>(stringArrays: T) => {
  ...
  Object.keys(stringArrays).forEach((k: keyof T) => {
    result[k] = stringArrays[k].join(CLASS_SEP);
  });
  return result;
};
...
export function extractMatches<C extends Record<string, string[]>>(
  classes: C,
  optimizeClassSwapping: boolean
) {
  ...
  let indexes: Record<string, number>,
    copy = copyArrObj<typeof classes, (typeof combineKeys)[number]>(
      classes,
      combineKeys
    );
  let result = {
