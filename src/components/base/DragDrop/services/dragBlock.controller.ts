...
function useDndController<DropItem, ThisItem>(
  type: Identifier,
  item: ThisItem | undefined,
  onDrop: DropHandler,
  ...
  const [{ isOver }, drop] = useDrop(
    () => ({
      ...
      drop: (dropped: DropItem, monitor) =>
        monitor.didDrop() || onDrop(dropped, item),
      collect: (monitor) => ({
...
export type DropHandler = <DropItem, ThisItem>(
  droppedItem: DropItem,
  thisItem: ThisItem
) => void;
export type DropTester = <DropItem, ThisItem>(
