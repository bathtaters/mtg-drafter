...
function getDefaultClasses(
  customBorder: Partial<BorderClasses>,
  customBgd: Partial<BgClasses>,
  additional: Partial<OtherClasses>
) {
  ...
  Object.keys(additional).forEach((c) => {
    if (!other[c]) return;
    else if (typeof additional[c] === "string")
      other[c].push(additional[c] as string);
    else if (Array.isArray(additional[c]))
      other[c].push(...(additional[c] as string[]));
  });
...
function toClassArray(
  base: Record<string, string>,
  type: ClassPrefix = "base",
  isBorder?: boolean
) {
  // Set defaults
  return [
...
export default function dragClassController(
  border: Partial<BorderClasses>,
  bgd: Partial<BgClasses>,
  other: Partial<OtherClasses>,
  droppable: boolean,
  draggable: boolean
) {
  // Get default classes
