...
export default function getAutoLands(
  ...
  if (landCount < 1)
    return `Main-board (${mainDeck.length}) is larger than deck size (${deckSize}). Lands weren't updated.`;
...
const blankCounter = sameValueObject<
  BoardLands & { lands: number; sum: number }
>([...colorOrder, "lands", "sum"], 0);
...
function countPips(cardList: GameCardFull[]) {
  ...
  cardList.forEach(({ card }) => {
    ...
    for (
      ...
      (match = pipRegEx.exec(card.manaCost));
  ...
  count.sum = Object.keys(count).reduce(
    (sum, c) => (c.length === 1 ? sum + count[c] : sum),
    0
  );
  return count;
}
