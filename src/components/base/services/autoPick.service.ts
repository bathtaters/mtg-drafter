...
const colorBase = () =>
  Object.fromEntries([...Object.keys(Color), NONE].map((c) => [c, 0]));

const getMaxValue = <T = any>(array: T[], sortAlgo: (a: T, b: T) => number) => {
  let max: T | undefined;
  array.forEach((entry) => {
    if (max === undefined || sortAlgo(max, entry) > 0) max = entry;
  });
  return max;
};
...
const getColorRating = (
  card: CardFull,
  colorRank: ReturnType<typeof colorBase>
) => {
...
export default function getAutopickCard(
  pack: PackFull,
  playerCards: GameCardFull[] = []
) {
  if (!playerCards.length)
    return getMaxValue(pack.cards, (a, b) => pickSort(a.card, b.card));

  // Order colors in player's main board from most to least
  const colorRank = playerCards
