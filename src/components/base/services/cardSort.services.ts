...
const sortValue: { [algo in SortKey]: SortValue } = {
  ...
  color: (card) =>
    card.colors.length === 1
      ...
      : !card.colors.length
        ? 11
        : 4 + card.colors.length,

  type: (card) =>
    Math.min(...card.types.map((type) => typeOrder.indexOf(type))),

  cost: (card) => card.manaValue || 0,
};

const raritySort: CardSorter = (a, b) =>
  sortValue.rarity(a) - sortValue.rarity(b);
...
export const deckSort: CardSorter = (a, b) =>
  colorSort(a, b) ||
  typeSort(a, b) ||
  costSort(a, b) ||
  nameSort(a, b) ||
...
export const pickSort: CardSorter = (a, b) =>
  raritySort(a, b) ||
  costSort(a, b) ||
  typeSort(a, b) ||
  nameSort(a, b) ||
