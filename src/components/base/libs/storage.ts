...
export const setLocalVar = <T = any>(
  key: LocalKeys,
  value: T,
  overwrite = true
) => {
  if (!overwrite && localStorage.getItem(storagePrefix + key) != null)
    return false;
  localStorage.setItem(storagePrefix + key, JSON.stringify(value));
  return true;
};

export const rmvLocalVar = (key: LocalKeys) =>
...
export function useLocalStorage<T = (typeof storageDefaults)[LocalKeys]>(
  key: LocalKeys
): [T, SetValue<T>, SetValue<T>] {
  const [state, setState] = useState<T>(storageDefaults[key] as T);

  useEffect(() => {
    if (typeof window !== "undefined") setState(getLocalVar<T>(key));
  }, [key]);
