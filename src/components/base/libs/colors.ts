...
function buildString(
  ...
  switch (prop) {
    ...
    case "bg":
    case "border":
      return `${hover ? "hover:" : ""}${prop}-${color}-${VALUES[+((prop === "bg") !== inverse)][+dim]}`;
    case "all":
      return `${buildString(color, "text", options)} ${buildString(color, "bg", options)} ${buildString(color, "border", options)}`;
    case "fg":
      return `${buildString(color, "text", options)} ${buildString(color, "border", options)}`;
  }
}

export default function getColorClass(
  id = -1,
  prop: Property,
  options: Partial<Options> = {}
) {
