...
export type PageRes<T = any> = { [dataArrayKey: string]: T[] } & {
  total: number;
  offset?: number;
  size?: number;
};
...
export const fetcher = <T = any>(
  ...
  fetch(input, init).then((res) =>
    res
      .json()
      .then((data: T) => ({ status: res.status, data }))
...
export async function paginatedFetcher<T = any>(
  url: string,
  {
    size = 100,
    key = "data",
  ...
  for (let total = size, offset = 0; offset < total; ) {
    const res = await fetcher<PageRes<T>>(
      `${url}?${objToQuery({ size, offset, ...params })}`
    );
...
const anyToString = (value: any): string | null =>
  value == null
    ? null
