...
export const canShare = () =>
  typeof window === "undefined"
    ? false
    : Boolean(window.navigator.share || window.navigator.clipboard?.writeText);

export default async function browserShare(
  text: string,
  url: string,
  title: string
): Promise<ShareResult> {
  if (
    ...
    (isMobileDevice() || !window.navigator.clipboard?.writeText)
  )
