...
export default function useTextEditor({
  ...
  useEffect(() => {
    if (typeof isEditing === "boolean") setEditingLocal(isEditing);
  }, [isEditing]);
  ...
  const handleChange: ChangeEventHandler<HTMLInputElement> = useCallback(
    (ev) => {
      setText(
        maxLength
          ? ev.currentTarget.value.slice(0, maxLength)
