...
import {
  ...
  RangeInputElem,
  RangeStepMarkers,
} from "./styles/RangeInputStyles";

const rangeValueError = (min: any, max: any, step: any) =>
  new Error(
  ...
  return (
    ...
    >
      <RangeInputElem
        ...
        onChange={
          setValue ? (ev) => setValue(ev.currentTarget.value) : undefined
        }
        {...props}
      />
      ...
      {caption && (
        <RangeStepMarkers
          first={keys ? undefined : min}
          last={keys ? undefined : max}
          count={length}
        />
      )}
    </RangeContainer>
