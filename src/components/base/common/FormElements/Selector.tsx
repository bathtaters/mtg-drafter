...
export default function Selector<ID extends string | number = number>({
  label,
  selected,
  setSelected,
  placeholder,
  ...
  return (
    <div
      className={`${label ? "tooltip" : ""} tooltip-secondary ${wrapperClass ?? ""}`}
      data-tip={label}
    >
      <select
        {...props}
        ...
        onChange={(ev) =>
          setSelected(
            ev.target.value === PLACEHOLDER
              ? undefined
              : (ev.target.value as ID)
          )
        }
      ...
      >
        ...
        {Children.map(children, (child, idx) => (
          <option key={child.key ?? idx} value={child.key ?? idx}>
            {child}
          </option>
        ))}
      </select>
