...
export const RangeContainer = ({ caption, value, tooltip, boxClass, captionClass = 'text-lg', className, children }: ContainerProps) => (
...
export const RangeInputElem = (props: HTMLProps<HTMLInputElement>) => (
  <input className="range range-sm md:range-md range-secondary disabled:cursor-not-allowed disabled:opacity-70" {...props} type="range" />
)
...
export function RangeStepMarkers({ first, last, count }: { first?: string | number, last?: string | number, count: number }) {
  return (
    <div className="w-full flex justify-between text-xs px-2">
      ...
      <span className="w-1 overflow-visible">{last === first ? null : last ?? MARKER_CHAR}</span>
    </div>
  )
}