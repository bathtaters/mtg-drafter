...
export function ErrorModalWrapper({
  alert,
  children,
  closeModal,
}: {
  alert: Required<ErrorAlert>;
  children: ReactNode;
  closeModal: () => void;
}) {
  ...
  return (
    <ModalWrapper
      ...
      setOpen={alert.bgdClick ? closeModal : undefined}
      isOpen={true}
...
export const errorButton = (
  ...
  return [
    <button
      type="button"
      className={`btn shadow-sm shadow-black ${buttonClasses[theme]}`}
      onClick={label === "Refresh" ? Router.reload : close}
      key={label}
    >
      {label === "Refresh" ? "⟳" : label}
    </button>,
...
export function ToastStyle({
  ...
  return (
    <div
      className={`alert flex-row items-center justify-start min-w-[50%] w-auto pointer-events-auto cursor-pointer opacity-75 flex-shrink flex-grow-0 ${colorClasses[theme]}`}
      onClick={onClick}
    >
      <Icon />
      {children}
