...
export function ErrorModal({
  ...
  return (
    <ErrorModalWrapper
      alert={fullAlert as Required<ErrorAlert>}
      closeModal={() => closeModal(fullAlert.id)}
    >
...
export function ToastDisplay({
  ...
  return (
    <ToastContainer>
      {list.map((alert) => (
        <ToastMessage key={alert.id} alert={alert} closeAlert={closeAlert} />
      ))}
    </ToastContainer>
  );
}
...
function ToastMessage({
  ...
  const { id, message, theme, disableClick, hideDelay } = {
    ...toastDefaults,
    ...alert,
  };
  ...
  return (
    <ToastStyle
      theme={theme}
      onClick={disableClick ? undefined : () => closeAlert(id)}
    >
      {message}
    </ToastStyle>
  );
