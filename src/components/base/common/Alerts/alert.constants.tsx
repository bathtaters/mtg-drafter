...
import {
  ...
  SuccessIcon,
  WarningIcon,
} from "components/svgs/AlertIcons";
...
export const buttonClasses: { [theme in AlertTheme]: string } = {
  ...
  warning:
    "btn-warning text-warning bg-warning-content hover:bg-warning-content/60",
  info: "btn-info text-info bg-info-content hover:bg-info-content/60",
  success:
    "btn-success text-success bg-success-content hover:bg-success-content/60",
  primary:
    "btn-primary text-primary bg-primary-content hover:bg-primary-content/60",
  secondary:
    "btn-secondary text-secondary bg-secondary-content hover:bg-secondary-content/60",
};

export const alertIcons: {
  [theme in AlertTheme]: (props: { className?: string }) => ReactElement<any>;
} = {
  base: InfoIcon,
  error: ErrorIcon,
  warning: WarningIcon,
...
type CallableConsoleKey = keyof {
  [P in keyof Console as Console[P] extends (...args: any[]) => void
    ? P
    : never]: any;
};
export const showInConsole: { [theme in AlertTheme]?: CallableConsoleKey } =
  clientErrorsInConsole
    ? {
        error: "error",
        warning: "warn",
      }
