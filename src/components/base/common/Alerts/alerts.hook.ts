...
const withId = <T extends { id?: string }>(obj: T) => ({
  id: nanoid(),
  ...obj,
});
...
export default function useAlerts(
  initialAlert: { error?: ErrorAlert[]; toast?: ToastAlert[] } = {}
) {
  const [errorList, updateErrors] = useGenericAlert(initialAlert.error);
  const [toastList, updateToasts] = useGenericAlert(
    initialAlert.toast,
    toastAdapter
  );
  ...
  return {
    ErrorComponent: () =>
      errorList[0] &&
      ErrorModal({ alert: errorList[0], closeModal: updateErrors.pop }),
    ToastComponent: () =>
      ToastDisplay({ list: toastList, closeAlert: updateToasts.pop }),
...
function useGenericAlert<Alert extends GenericAlert>(
  initialAlert: Alert[] = [],
  adapter: AlertAdapter<Alert> = withId
) {
  ...
  const push = useCallback(
    (alert: Alert) => {
      ...
      console[showInConsole[alert.theme as "info"] as "log"]?.(alert.message);
      return apdapted.id;
    },
    [adapter, pop]
  );
...
type AlertAdapter<Alert extends GenericAlert> = (
  alert: Alert,
  pop: (withId?: string) => void
) => Alert;
