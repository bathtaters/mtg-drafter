...
export default function useSetupController() {
  ...
  const disableSubmit =
    !options.name ||
    (options.type === "Cube" ? !file : !options.packList.length);
  const submitForm: FormEventHandler | undefined = disableSubmit
    ? undefined
    : (ev) => {
        ev.preventDefault();
        return createGame(options, file);
      };

  const setPack = (idx: number) => (code?: string) => {
    code && setOption.packList(spliceInPlace(options.packList, idx, 1, code));
  };

  const addPack =
    options.packList.length < setupLimits.packs.max &&
    (() =>
      setOption.packList(
        options.packList.concat(options.packList[options.packList.length - 1])
      ));
  const rmvPack =
    options.packList.length > setupLimits.packs.min &&
    (() =>
      setOption.packList(
        options.packList.slice(0, options.packList.length - 1)
      ));
