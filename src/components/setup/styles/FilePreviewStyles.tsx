...
export const PreviewResult = ({ children }: { children: ReactNode }) => (
  <div className="w-full text-center text-xs text-[color-mix(in_oklab,oklch(var(--s)),black_10%)] italic">
    {children}
  </div>
);

export const PreviewError = ({ error }: { error?: string }) => (
  <div className="w-full text-center text-xs text-error italic">
    Error Uploading: {error || "Unknown Error"}
  </div>
);

export const NotFoundList = ({ list }: { list?: string[] }) =>
  !list?.length ? null : (
...
export const ClearButton = ({
  ...
  <button
    type="button"
    className="block w-1/2 btn btn-secondary btn-sm"
    {...props}
  >
    {label}
  </button>
