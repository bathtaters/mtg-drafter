...
export const newGameURL = (type: DraftType) =>
    type === "Cube" ? "/api/setup/cube" : "/api/setup/booster",
  cubeListURL = "/api/setup/upload",
  gameURL = (gameIdentifier: string) => `/game/${gameIdentifier}`,
  gameAPI = (gameURL: any) =>
    `/api/game/${typeof gameURL === "string" ? gameURL : INVALID_PATH}/all`,
  socketEndpoint = (gameURL: any) =>
    `/api/game/${typeof gameURL === "string" ? gameURL : INVALID_PATH}/socket`,
  scryfallLink = (scryfallId: string) =>
    `https://scryfall.com/card/${scryfallId}`,
  scryfallImageUrl = (
    sfId: string,
    front = true,
    fmt: ScryfallImgFmt = "large"
  ) =>
