...
export const roundCounter = (
  ...
  isNotJoined || !status || !game || !("round" in game)
    ? "Waiting Room"
    : status === "start"
      ? "Starting Soon"
      : status === "end"
...
export const cardCounter = (count?: number, lands?: BoardLands) =>
  typeof count !== "number" || (!count && !lands)
    ? undefined
    : lands
      ? `${count} | ${getObjectSum(lands) + count}`
      : `${count}`;

export const pickInfoText = (
  { name, pick, pack }: PickInfo[string],
  isDeck: boolean
) => {
...
export const sharingMessage: Record<string, ToastAlert> = {
  ...
  error: {
    message: "Unable to share link, refresh page then try again",
    theme: "error",
  },
  unavailable: {
    message: "Link sharing is not available in your browser",
    theme: "error",
  },
};

export const maxSizeError = (size: number, maxSize: number) =>
  `File exceeds ${formatBytes(maxSize)} limit (${formatBytes(size)})`;

export const timerAlertMsg = (sec: number) =>
    `Under ${sec} seconds left to pick!`,
...
export const logOptionLabels: Record<
  keyof LogOptions | "showSidebar",
  ReactNode
> = {
  hideHost: (
    <>
      <span>Show Host</span>
      <HostIcon className="ml-2 w-5 ms-2x" />
    </>
  ),
  hidePrivate: (
    <>
      <span>Show Secrets</span>
      <CardIcon className="ml-2 w-5 stroke-current fill-primary-content inline" />
    </>
  ),
  hideWatchers: (
    <>
      <span>Show Watchers</span>
      <WatcherIcon className="ml-2 w-5 fill-current inline" />
    </>
  ),
  showSidebar: (
    <>
      <span>Show Players</span>
      <UserIcon className="ml-2 w-5 fill-current inline" />
    </>
  ),
};

export const logFullDate = (dt: Date) =>
  dt.toLocaleString(undefined, { timeStyle: "medium", dateStyle: "medium" });

export const logTimestamp = (dt: Date) =>
  dt
...
export const logFilename = (gameUrl: string) =>
  `drafter_log_${gameUrl}_${new Date().toLocaleDateString("en-CA").replaceAll("-", "")}`;
export const LOG_EXT = ".json";

export const formatLogAction = (
  action: LogAction,
  data: LogData,
  hostId: Game["hostId"],
  ...
  switch (action) {
    ...
    case "view":
      ...
      return isNaN(+data)
        ? `${data}board viewed`
        : `pack-${data.padStart(2, "0")} viewed`;

    case "join":
      return hostId ? "added" : "joined";
    case "leave":
      return !hostId ? "left" : data === ALL_WATCHERS ? "cleared" : "removed";

    case "rename":
      return hostId ? "renamed" : "renamed";
    ...
    case "settings":
      ...
      return ` ${
        Object.entries(gameData)
          .map(([key, val]) =>
            key === "timerBase"
              ? `timer ${!val ? "disabled" : `set to "${timerText[+val]?.value || val}"`}`
              : key === "hostId"
                ? "became host"
    ...
    case "round":
      return data === "END" ? "Ended" : `Round ${data || "?"}`;

    case "pause":
      return data ? "Resumed" : "Paused";
